...
class RightMenu extends React.Component {
  ...
  delete = () => {
    ...
    fetch(`https://i.cs.hku.hk/~wyvying/php/profile/delete.php?commentID=${encodeURIComponent(parseInt(Comment_ID))}&category=${encodeURIComponent(category)}`, {
        method: "GET", // *GET, POST, PUT, DELETE, etc.
    })
    .then(response => response.json()) // parses response to JSON
    .then((data) => {
      if (data.state == "success"){
        AsyncStorage.setItem('Update', '1').then(() => this.props.navigation.pop());
      }
  ...
  deleteAlert = () => {
    this._menu.hide();
    Alert.alert( '你確定要刪除此評論?', '', [
      {text: "是", onPress: () => this.delete()},
    {text: '不了', onPress: () => console.log('No Pressed')}]);
  };
  ...
  edit = () => {
    ...
    totalrating = parseInt(rating_1) + parseInt(rating_2) + parseInt(rating_3) + parseInt(rating_4);
    var rating = (totalrating / 4).toFixed(0);

      this.props.navigation.navigate('EditComment',
      {CommentId: Comment_ID, Topic: topic, Rating_1: rating_1, Rating_2: rating_2, Rating_3: rating_3, Rating_4: rating_4,
        Comment: comment, image_num: image_num, category: category, rating: rating.toString()});
        ...
        <Menu
            ...
            button={
              <TouchableOpacity onPress={() => this._menu.show()}>
                <FastImage style= {{width: 20, height: 20}} source={require('../../assets/menu.png')} />
              </TouchableOpacity>
            }
        >
          <MenuItem onPress={() => this.deleteAlert()}>刪除評論</MenuItem>
          <MenuItem onPress={() => this.edit()}>修改評論</MenuItem>
        </Menu>
      </View>
    );
  }
