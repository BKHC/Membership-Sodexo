...
export default class MyCommentList extends React.Component {
  ...
  _doFetch = () => {
    AsyncStorage.getItem('userID').then((value) => {
      fetch(`https://i.cs.hku.hk/~wyvying/php/profile/mylist.php?userID=${encodeURIComponent(value)}`, {
    ...
    _renderData = ({item}) => (
      ...
      >
        ...
        <View style={{flexDirection:'row', justifyContent : 'space-between'}} >
          ...
          {item.category == "0" ?
          (<CanLikeStatus commentID={item.id} />) : item.category == "1" ?
          (<HallLikeStatus commentID={item.id} />):
          (<CouLikeStatus commentID={item.id} />)}
        </View>
