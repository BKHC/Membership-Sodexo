<?php
	...
	for($x = 1; $x <= $i; $x++)
	{
		$temp_query_1 = "SELECT AVG(`Rating_1`) FROM `Course Rate` WHERE `CourseID`= $x";
		$temp_qr_1 = mysqli_query($db, $temp_query_1);
		$temp_result_1 =	mysqli_fetch_assoc($temp_qr_1);
    
		$temp_query_2 = "SELECT AVG(`Rating_2`) FROM `Course Rate` WHERE `CourseID`= $x";
		$temp_qr_2 = mysqli_query($db, $temp_query_2);
		$temp_result_2 =	mysqli_fetch_assoc($temp_qr_2);
    
		$temp_query_3 = "SELECT AVG(`Rating_3`) FROM `Course Rate` WHERE `CourseID`= $x";
		$temp_qr_3 = mysqli_query($db, $temp_query_3);
		$temp_result_3 =	mysqli_fetch_assoc($temp_qr_3);
    
		$temp_query_4 = "SELECT AVG(`Rating_4`) FROM `Course Rate` WHERE `CourseID`= $x";
